...
def getFlux(rho_L, rho_R, vx_L, vx_R, vy_L, vy_R, P_L, P_R, gamma):
	...
	return np.stack([flux_Mass, flux_Momx, flux_Momy, flux_Energy], axis=0)


def dask_v(N = 256,whether_plot=True,chunksize=128):
	...
	while t < tEnd:
		...
		# compute fluxes (local Lax-Friedrichs/Rusanov)
		res =[]
		rho_XL_dask, rho_XR_dask = da.from_array(rho_XL, chunks=chunk), da.from_array(rho_XR, chunks=chunk)
		vx_XL_dask, vx_XR_dask = da.from_array(vx_XL, chunks=chunk), da.from_array(vx_XR, chunks=chunk)
		vy_XL_dask, vy_XR_dask = da.from_array(vy_XL, chunks=chunk), da.from_array(vy_XR, chunks=chunk)
		P_XL_dask, P_XR_dask = da.from_array(P_XL, chunks=chunk), da.from_array(P_XR, chunks=chunk)

		res.append(da.map_blocks(getFlux,rho_XL_dask, rho_XR_dask, vx_XL_dask, vx_XR_dask, vy_XL_dask, vy_XR_dask, P_XL_dask, P_XR_dask, gamma,dtype=rho_XL_dask.dtype,chunks=(4,chunksize,chunksize)))
		

		#flux_Mass_Y, flux_Momy_Y, flux_Momx_Y, flux_Energy_Y = getFlux(rho_YL, rho_YR, vy_YL, vy_YR, vx_YL, vx_YR, P_YL, P_YR, gamma)

		rho_YL_dask, rho_YR_dask = da.from_array(rho_YL, chunks=chunk), da.from_array(rho_YR, chunks=chunk)
		vx_YL_dask, vx_YR_dask = da.from_array(vx_YL, chunks=chunk), da.from_array(vx_YR, chunks=chunk)
		vy_YL_dask, vy_YR_dask = da.from_array(vy_YL, chunks=chunk), da.from_array(vy_YR, chunks=chunk)
		P_YL_dask, P_YR_dask = da.from_array(P_YL, chunks=chunk), da.from_array(P_YR, chunks=chunk)

		res.append(da.map_blocks(getFlux,rho_YL_dask, rho_YR_dask, vy_YL_dask, vy_YR_dask, vx_YL_dask, vx_YR_dask, P_YL_dask, P_YR_dask, gamma,dtype=rho_YL_dask.dtype,chunks=(4,chunksize,chunksize)))

		res1,res2=da.compute(*res)
		flux_Mass_X, flux_Momx_X, flux_Momy_X, flux_Energy_X = res1[0],res1[1],res1[2],res1[3]
		flux_Mass_Y, flux_Momy_Y, flux_Momx_Y, flux_Energy_Y = res2[0],res2[1],res2[2],res2[3]
